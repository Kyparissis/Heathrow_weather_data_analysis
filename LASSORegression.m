% This function finds the optimal model for the depended variable using LASSO regression method.
% The optimal model is the one with the maximum adjusted R2 value.
% INPUTS:
% dependedVariable: A vector of the depended variable.
% independedVariables: A matrix of the independed variables.
% OUTPUTS:
% OptimalModel_bin: The optimal model as a logical vector with length equal to the number of columns in independedVariableSamples, indicating which terms are in the final model.
% OptimalModel_lambda: The lambda value that gives the minimum MSE for the optimal model.
function [OptimalModel_bin, OptimalModel_lambda] = LASSORegression(dependedVariable, independedVariables)
